...
void print(cell** t, size_t m, size_t n, ssize_t** assignment
		, int*ret_val
);
...
//    fclose(urandom);
    ...
    size_t i, j;
    size_t n = int_array_n;
    size_t m = int_array_m;
...
//	        *(*(table + i) + j) = *(*(t + i) + j) = (cell)(random() & 63);
	        *(*(table + i) + j) = *(*(t + i) + j) = (cell)(*(int_array_elems + i * m + j));
	}
//    else
//    {
//        cell x;
...
//    }
    ...
    if (VERBOSE_HUNGARIAN) printf("\nInput:\n\n");
    print(t, n, m, null, null);
    
    ssize_t** assignment = kuhn_match(table, n, m);
    if (VERBOSE_HUNGARIAN)printf("\nOutput:\n\n");
    print(t, n, m, assignment, ret_val);
...
void print(cell** t, size_t n, size_t m, ssize_t** assignment, int *retval)
{
    size_t i, j;
	...
	for (j = 0; j < m; j++)
	{
	    if (*(*(assigned + i) + j))
	    {
		    ...
		    if (VERBOSE_HUNGARIAN) printf("\033[%im", (int)(30 + *(*(assigned + i) + j)));
	    }
	    if (VERBOSE_HUNGARIAN) printf("%5li%s\033[m   ", (cell)(*(*(t + i) + j)), (*(*(assigned + i) + j) ? "^" : " "));
        }
	if (VERBOSE_HUNGARIAN) printf("\n\n");
